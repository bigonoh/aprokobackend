...
const acceptRejectProposal = catchAsync(async (req, res) => {
  const { status, proposal_id } = req.body;
  try {
    ...
    const dProposal = await Proposal.findOne({ _id: proposal_id }).populate('answered_info_id').populate('ask_info_id');
    ...
    if (dProposal) {
      ...
      if (sales) {
        ...
        const dt = await debitWallet({ ...transaction, user_id: transaction.user });
    ...
    const proposals = await proposalService.updateProposalById(dProposal.id, { status, accepted: status === 'accepted' });

    return success(
      res,
      status === 'accepted' ? 'Proposal Accepted successfully' : 'Proposal rejected succesfully',
      proposals
    );
  } catch (error) {
