...
const transfer = async (req, res) => {
    ...
    try {
      ...
      const transferResult = await Promise.all([
        debitWallet(
          {amount, user_id:from, trx_status: 'success', purpose:"Transfer", trx_ref, summary: `You sent ${formatNaira(amount)} to ${to}`,
          trx_summary: `TRFR TO: ${to}. TRX REF:${trx_ref} `, session}),
        creditWallet(
          {amount, trx_status: 'success', user_id:toUser._id, toUser, purpose:"Transfer", trx_ref, summary: `${fromUser}, Sent ${formatNaira(amount)} to you `,
          trx_summary:`TRFR FROM: ${fromUser}. TRX REF:${trx_ref} `, session})
      ]);
