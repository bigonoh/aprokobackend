const input_names = ['amount', 'account_number', 'type', 'email','first_name','last_name','phone','auth_key', 'source', 'session_id', 'business_name', 'account_bank', 'account_name', 'bank', 'bvn'];
        ...
        if (inputs.length > 0) {
            ...
            inputs.map((input) => {
                ...
                if (reqBody[input]) {
                    ...
                    let paramIndex = input_names.indexOf(input),
                        paramType = typeof reqBody[input],
                        intendedParamType = input_types[paramIndex] || 'string';

                    if (!intendedParamType.includes(paramType)) {

                        error = true
                        message += ((message.length > 0) ? ' || ' : '') + '(' + input + ') needs to be of type ' + intendedParamType + ', detected ' + paramType
                    } else {
...
function authHeader(req) {
    ...
    if (req.headers) {
        ...
        if (auth === undefined)
            return ('no authorization header parsed')
        else {
...
async function encryptedBad(res, message) {
    let encryptedResponse = await encrypt(message)
